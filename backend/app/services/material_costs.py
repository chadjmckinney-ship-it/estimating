...
def _panel_lines(db: Session, section: EstimateSection) -> list[MaterialLine]:
    """Panel concrete by mix and the steel, the way the section was costed (sql/077)."""
...
def _misc_lines(db: Session, section: EstimateSection) -> list[MaterialLine]:
    """The items' concrete by mix and their steel, the way the section was costed (sql/078)."""
...
def _section_material_costs(db: Session, section: EstimateSection) -> dict[str, Any]:
    ...
    if section.kind in PIER_KINDS:
        ...
        lines = _panel_lines(db, section)
    elif section.kind in MISC_KINDS:
        lines = _misc_lines(db, section)
    else:
